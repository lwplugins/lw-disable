...
(function () {
	 ...
	 */
	function initTabs() {
		var tabLinks  = document.querySelectorAll( '.lw-disable-tabs a' );
		var tabPanels = document.querySelectorAll( '.lw-disable-tab-panel' );
		...
		var form = document.querySelector( '.lw-disable-settings' );
		if (form) {
		...
		if (form) {
			form.addEventListener(
				'submit',
				function () {
					var activeLink = document.querySelector( '.lw-disable-tabs a.active' );
					if ( ! activeLink) {
						return;
					}
					var tabId    = activeLink.getAttribute( 'href' ).substring( 1 );
					var tabInput = form.querySelector( 'input[name="lw_disable_active_tab"]' );
					if (tabInput) {
